...
export default function Sidebar({ active, onNav, stats, syncStatus, configured, canEdit, viewerMode }) {
  ...
  return (
    <aside style={{ width:200, minWidth:200, background:'#1c1814', display:'flex', flexDirection:'column', borderRight:'1px solid #2e2820' }}>
      <div style={{ padding:'24px 20px 16px', borderBottom:'1px solid #2e2820' }}>
        ...
        {sync.label && (
          <div style={{ marginTop:6, fontSize:10, fontWeight:500, color:sync.color, letterSpacing:'0.04em' }}>{sync.label}</div>
        )}
        {!configured && canEdit && (
